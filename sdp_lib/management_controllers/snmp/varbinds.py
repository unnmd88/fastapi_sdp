...
ug405_stage_vaules = convert_val_to_num_stage_set_req(128)
print(f'ug405_stage_vaules 1: {ug405_stage_vaules.get("8")}')
pprint.pprint(ug405_stage_vaules)
...
class Varbinds(abc.ABC):
    ...
    def get_varbinds_get_state(self):
...
class VarbindsStcip(Varbinds):
    ...
    def get_varbinds_get_state(self) -> list[ObjectType]:
...
#     num_CO_prefix = 'CO'
#
#     operation_mode2_varbind = wrap_oid_by_object_type(Oids.utcType2OperationMode, Integer(2))
#     operation_mode3_varbind = wrap_oid_by_object_type(Oids.utcType2OperationMode, Integer(3))
#     integer_vals = {i: Integer(i) for i in range(128)}
#     hex_vals = {i: OctetString(str(i)) for i in range(128)}
#
#     def __init__(self):
#         super().__init__()
#         self._varbinds_get_state_all_scn = self._build_varbinds_for_get_state()
#
...
#     ) -> Oids | str | ObjectType:
#
#         oid = f'{oid}{scn}' if oid in oids.oids_scn_required else oid
#         if wrap_oid_by_object_identity:
#             return wrap_oid_by_object_type(oid)
#         return  oid
#
#     def _build_operation_mode_varbinds(self, mode: int):
#         return wrap_oid_by_object_type(Oids.utcType2OperationMode, self.integer_vals.get(mode))
#
#     def _build_varbinds_for_get_state(self):
...
#             curr_states_object_type = self.add_scn_to_oids_and_wrap_oids_by_object_identity(
#                 scn, self.oids_state
#             )
...
#         return result
#
#     def build_set_stage_oids_by_scn(self, scn_as_ascii):
#
#         res = {}
#         for i in range(1, self.max_stage):
#         # for i in range(1, 3):
#             op_mode = wrap_oid_by_object_type(Oids.utcType2OperationMode, Integer(3))
#
#             TO_bit = wrap_oid_by_object_type(
#                 self.add_scn_to_single_oid(scn_as_ascii, Oids.utcControlTO), Integer(1)
#             )
#             Fn_bit = wrap_oid_by_object_type(
#                 self.add_scn_to_single_oid(scn_as_ascii, Oids.utcControlFn), OctetString(hexValue=str(i))
#             )
#             res[i] = (op_mode, TO_bit, Fn_bit)
#         return res
#
#     def _build_varbinds_for_set_stage(self):
#         start_time = time.time()
#
#         result = {}
#         for i in range(1, self.max_scn):
...
#
#             scn = convert_chars_string_to_ascii_string(f'{self.num_CO_prefix}{str(i)}')
#             result[scn] = self.build_set_stage_oids_by_scn(scn)
#             print(f'one iteration : {time.time() - start_time}')
#
#         print(f'final_time: {time.time() - start_time}')
#
#         return result
#
#     def add_scn_to_oids(
...
#             oids: T_OidsContainer
#     ) -> T_OidsContainer:
#         return self._add_scn_to_oids(scn, oids)
#
#     def add_scn_to_oids_and_wrap_oids_by_object_identity(
#             self,
#             scn: str,
#             oids: T_OidsContainer
#     ) -> T_ObjectTypeContainer:
#         return self._add_scn_to_oids(scn, oids, wrap_oid_by_object_identity=True)
#
#     def get_varbinds_get_state_by_scn(self, scn_as_ascii: str) -> T_ObjectTypeContainer:
#         return self._varbinds_get_state_all_scn.get(scn_as_ascii)
#
#     def get_varbinds_set_stage(self, scn_as_ascii, num_stage):
#         return self._varbinds_set_stage.get(scn_as_ascii).get(num_stage)
#
#     def _build_hex_vals_stages(self):
#         return {num_stage: OctetString(hexValue=str(num_stage)) for num_stage in range(1, 129)}
#
#     def get_stage_as_hex(self, num_stage_as_integer):
#         return self._hex_values_stages.get(num_stage_as_integer, OctetString(hexValue='0'))
...
class VarbindsUg405(Varbinds):
    ...
    hex_vals128 = {i: OctetString(hexValue=ug405_stage_vaules.get(str(i))) for i in range(1, 129)}
...
async def main():
    ...
    scn2600 = convert_chars_string_to_ascii_string('CO2600')
    # res = await r_sender.snmp_get(varbinds=potok_ug405.get_varbinds_current_states(scn155))
    obj = WrappersVarbindsByScnPotokP(scn2600)
    # res = await r_sender.snmp_get(varbinds=obj.get_varbinds_current_states())
    print(f's: {obj.ug405_varbinds.get_varbinds_current_states(scn2600)}')
    print(f's: {obj.ug405_varbinds._varbinds_get_state.get(scn2600)}')
    res = await r_sender.snmp_get(varbinds=obj.ug405_varbinds.get_varbinds_current_states(scn2600))
    print(res)
    print(VarbindsSwarco.parse_response(res[3]))

    res2 = await r_sender.snmp_set(varbinds=obj.get_varbinds_set_stage(0))
    print(VarbindsSwarco.parse_response(res2[3]))
